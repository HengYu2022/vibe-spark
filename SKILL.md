...
- **选了一个灵感** → 输出 `👍 好选择！` → 用 Read 工具读取该灵感的完整内容 → 进入 Phase 3
- **「我有自己的想法」** → 输出 `💡 有自己的想法很棒！说来听听：` → 让用户输入想法 → 在灵感库中搜索相关灵感展示作为参考 → 用最多 2 轮引导对话帮用户细化（每轮开头先肯定用户的想法，如「这个方向有意思」「想法越来越清晰了」）：
  - 第 1 轮：「这个想法最酷的版本是什么？做到什么程度算 MVP？」
  - 第 2 轮：「技术栈有偏好吗？大概想花多少时间？」
  → 进入 Phase 3
- **「换几个」** → 输出 `🔄 没关系，换一批看看！` → 重新匹配，排除已展示的灵感（最多换 2 轮）
- **换完 2 轮仍不满意** → 输出 `🤔 还没找到心动的？没关系，好灵感值得多找找。` → 问用户：「要放宽条件看更多，还是说说你自己的想法？」
